...
void set_twiststamp(T & out)
{
    if (!use_imu_as_input)
    {
        out.twist.linear.x = kf_output.x_.vel(0);
 	out.twist.linear.y = kf_output.x_.vel(1);
	out.twist.linear.z = kf_output.x_.vel(2);
    }
    else
    {
        out.twist.linear.x = kf_input.x_.vel(0);
 	out.twist.linear.y = kf_input.x_.vel(1);
	out.twist.linear.z = kf_input.x_.vel(2);
    }
}
